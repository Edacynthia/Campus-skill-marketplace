                ...
                @if(auth()->check() && $userApplication)
                    <div class="bg-blue-50 border border-blue-200 rounded-xl p-6">
                        ...
                        <p class="text-blue-800">
                            ...
                            Your application is currently <span class="font-semibold">{{ $userApplication->status }}</span>.
                        </p>
                    </div>
                @endif
            </div>
                        ...
                        <div class="flex items-center gap-4 mb-4">
                            ...
                            @else
                                <div class="w-16 h-16 bg-gradient-to-br from-blue-100 to-blue-200 rounded-full flex items-center justify-center">
                                    <span class="text-lg font-bold text-[#1e3a8a]">{{ substr($job->employer->first_name, 0, 1) }}{{ substr($job->employer->last_name, 0, 1) }}</span>
                                </div>
                            @endif
                            <div>
                                <p class="font-semibold text-gray-900">{{ $job->employer->first_name }} {{ $job->employer->last_name }}</p>
                                <p class="text-sm text-gray-500">{{ $job->employer->department ?? 'University Staff' }}</p>
                ...
                @if(auth()->check())
                    @if($job->employer_id === auth()->id())
                        ...
                        <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                            ...
                            <a href="{{ route('dashboard') }}" class="w-full px-4 py-2 bg-emerald-600 text-white font-semibold rounded-lg hover:bg-emerald-700 transition-all text-center block">
                    ...
                    @elseif($userApplication)
                        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
                            <p class="text-center text-gray-600 font-medium">Application Already Submitted</p>
                        </div>
                    ...
                    @else
                        <button onclick="showApplicationForm()" class="w-full px-6 py-3 bg-[#1e3a8a] text-white font-semibold rounded-xl hover:bg-[#0f2b5e] transition-all shadow-sm hover:shadow-md">
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6">
                        ...
                        <div class="space-y-3">
                            @foreach($relatedJobs as $relatedJob)
                                <a href="{{ route('jobs.show', $relatedJob->id) }}" class="block p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-all">
                                    <div class="flex items-center gap-3">
                                        @if($relatedJob->image)
                                            <img src="{{ asset('storage/' . $relatedJob->image) }}" alt="{{ $relatedJob->title }}" 
                                        ...
                                        @endif
                                        <div class="flex-1">
                                            <p class="font-medium text-gray-900 text-sm">{{ Str::limit($relatedJob->title, 30) }}</p>
                                            <p class="text-sm text-[#1e3a8a] font-semibold">{{ $relatedJob->formatted_salary }}</p>
                                        </div>
                                    </div>
                                </a>
                            @endforeach
            ...
            <div class="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <form action="{{ route('jobs.apply', $job->id) }}" method="POST" class="p-6">
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="button" onclick="hideApplicationForm()" class="px-6 py-3 border border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-all">
                            Cancel
                        </button>
                    </div>
                </form>
