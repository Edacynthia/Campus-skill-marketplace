                ...
                <form method="POST" action="{{ route('jobs.store') }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" required
                            placeholder="e.g., Research Assistant, Campus Photographer" value="{{ old('title') }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="category" required
                                ...
                                <option value="teaching" {{ old('category') == 'teaching' ? 'selected' : '' }}>Teaching
                                </option>
                        ...
                        <div>
                            ...
                            <select name="type" required
                                ...
                                <option value="on_campus" {{ old('type') == 'on_campus' ? 'selected' : '' }}>On Campus
                                </option>
                                <option value="off_campus" {{ old('type') == 'off_campus' ? 'selected' : '' }}>Off Campus
                                </option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="salary" required min="0" step="1" placeholder="5000"
                                value="{{ old('salary') }}"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:border-[#1e3a8a] transition-all">
                            @error('salary')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="location" required
                            placeholder="e.g., Faculty of Engineering, Library, Main Campus" value="{{ old('location') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:border-[#1e3a8a] transition-all">
                        @error('location')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <button type="button" onclick="addRequirement()"
                            class="text-[#1e3a8a] hover:text-[#0f2b5e] text-sm font-medium">
                            <i class="fa-solid fa-plus"></i> Add Requirement
                        </button>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Job Image</label>

                        <input type="file" name="image" accept="image/*"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:border-[#1e3a8a] transition-all">

                        <p class="text-xs text-gray-500 mt-1">
                            Upload an image that represents this job. JPG, PNG, WEBP. Max 2MB.
                        </p>
