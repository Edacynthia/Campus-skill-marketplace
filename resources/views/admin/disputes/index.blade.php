...
<div class="max-w-7xl mx-auto px-4 py-8">
                ...
                <div class="flex justify-between items-start gap-4">
                    ...
                    <div>
                        <span class="px-3 py-1 rounded-full text-xs font-semibold
                            @if(($booking->dispute_status ?? 'open') === 'open')
    ...
    <div class="mt-5 p-4 bg-green-50 border border-green-200 rounded-lg">
        ...
        @if($booking->payment_status === 'provider_confirmed_received')
            <p class="text-sm text-green-700 mt-1">
                Provider has confirmed that payment was received.
            </p>
        @endif
...
@else
    <div class="flex flex-wrap gap-2 mt-5">
        <form action="{{ route('admin.disputes.warnClient', $booking->id) }}" method="POST">
            @csrf
            <button class="px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm">
                Warn Client
            </button>
        </form>

        <form action="{{ route('admin.disputes.resolve', $booking->id) }}" method="POST">
            @csrf
        ...
        <form action="{{ route('admin.disputes.dismiss', $booking->id) }}" method="POST">
            @csrf
            <button class="px-3 py-2 bg-gray-700 hover:bg-gray-800 text-white rounded-lg text-sm">
                Dismiss Dispute
            </button>
        </form>

        <form action="{{ route('admin.users.suspend', $booking->client) }}" method="POST"
              onsubmit="return confirm('Are you sure you want to suspend this client?');">
            @csrf
            <button class="px-3 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg text-sm">
                Suspend Client
            </button>
        </form>

        <form action="{{ route('admin.users.suspend', $booking->provider) }}" method="POST"
              onsubmit="return confirm('Are you sure you want to suspend this provider?');">
            @csrf
            <button class="px-3 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg text-sm">
                Suspend Provider
            </button>
        ...
        <form action="{{ route('admin.users.ban', $booking->client) }}" method="POST"
              onsubmit="return confirm('Are you sure you want to ban this client?');">
            @csrf
            <button class="px-3 py-2 bg-red-700 hover:bg-red-800 text-white rounded-lg text-sm">
                Ban Client
            </button>
        </form>

        <form action="{{ route('admin.users.ban', $booking->provider) }}" method="POST"
              onsubmit="return confirm('Are you sure you want to ban this provider?');">
            @csrf
            <button class="px-3 py-2 bg-red-700 hover:bg-red-800 text-white rounded-lg text-sm">
                Ban Provider
            </button>
