...
@section('content')
        ...
        <div class="max-w-6xl mx-auto px-6">
            ...
            @if($messages->count() > 0)
                <div class="mb-4 p-3 bg-blue-50 rounded-lg">
                    <p class="text-sm text-blue-800">Debug: Found {{ $messages->count() }} messages</p>
                    <p class="text-xs text-blue-600">Current User ID: {{ auth()->id() }}</p>
                </div>
                <div class="space-y-4">
                    @foreach($messages as $message)
                        <div class="mb-2 p-2 bg-gray-50 rounded text-xs">
                            Debug: Message ID {{ $message->id }} - Sender: {{ $message->sender_id }} - Receiver: {{ $message->receiver_id }} - Status: {{ $message->status }}
                        </div>
                                ...
                                <div class="flex items-center justify-between">
                                        ...
                                        <div>
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if($message->sender->id != auth()->id())
                                                    <span class="text-xs text-gray-500">→ {{ $message->receiver->first_name }} {{ $message->receiver->last_name }}</span>
                                                @endif
                                            </div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        @if($message->status === 'sent' && $message->receiver_id == auth()->id())
                                            <span class="px-2 py-1 bg-blue-50 text-blue-600 text-xs font-medium rounded">New</span>
                                        @elseif($message->status === 'read')
                                            <span class="px-2 py-1 bg-gray-50 text-gray-600 text-xs font-medium rounded">Read</span>
                                        @elseif($message->status === 'replied')
                                            <span class="px-2 py-1 bg-green-50 text-green-600 text-xs font-medium rounded">Replied</span>
                                        @endif
                                ...
                                @if($message->skill_id && $message->skill)
                                    <div class="mb-3 p-3 bg-blue-50 rounded-lg">
                                        <div class="flex items-center gap-2 text-sm">
                                            ...
                                            <span class="text-blue-800">Regarding: <strong>{{ $message->skill->title }}</strong></span>
                                        </div>
                                    </div>
                                @elseif($message->job_id && $message->job)
                                    <div class="mb-3 p-3 bg-green-50 rounded-lg">
                                        <div class="flex items-center gap-2 text-sm">
                                            ...
                                            <span class="text-green-800">Regarding: <strong>{{ $message->job->title }}</strong></span>
                                        </div>
                                    </div>
                                @endif
                                    ...
                                    <div class="flex items-center gap-2">
                                        @if($message->receiver_id == auth()->id() && $message->status === 'sent')
                                            <form action="{{ route('messages.read', $message->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="text-xs px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-all">
                                                    Mark as Read
                                                </button>
                                            </form>
                                        @endif
                                        
                                        @if($message->receiver_id == auth()->id() && $message->status !== 'replied')
                                            <button onclick="openReplyModal({{ $message->id }}, '{{ $message->sender->first_name }}', '{{ $message->message }}')" 
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($message->sender_id == auth()->id())
                                            <form action="{{ route('messages.destroy', $message->id) }}" method="POST" class="inline" 
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-xs px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition-all">
                                                    Delete
                                                </button>
                                            </form>
                                        @endif
            ...
            <div class="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
                ...
                <form action="{{ route('messages.reply', '__id__') }}" method="POST" class="p-6" id="replyForm">
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="button" onclick="closeReplyModal()" class="px-4 py-3 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-all">
                            Cancel
                        </button>
                    </div>
                </form>
    ...
    function openReplyModal(messageId, senderName, originalMessage) {
        ...
        // Update form action with correct message id
        const form = document.getElementById('replyForm');
        form.action = form.action.replace('__id__', messageId);
    }
