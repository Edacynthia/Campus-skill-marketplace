...
@section('content')
                                    ...
                                    @if ($application->escrow_status === 'funded')
                                        <div class="mb-4 bg-green-50 border border-green-200 rounded-xl p-4">
                                            ...
                                            <p class="text-sm text-green-700 mt-2">
                                                Payment has been secured. The worker may now begin work.
                                            </p>
                                        </div>
                                    @elseif($application->escrow_status === 'released')
                                        <div class="mb-4 bg-blue-50 border border-blue-200 rounded-xl p-4">
                                            ...
                                            <p class="text-sm text-blue-700 mt-2">
                                                Job completed successfully. Payment has been released to the worker.
                                            </p>
                                        </div>
                                    @elseif($application->escrow_status === 'completed')
                                        <div class="mb-4 bg-amber-50 border border-amber-200 rounded-xl p-4">
                                            ...
                                            <p class="text-sm text-amber-700 mt-2">
                                                The worker has completed the job. Review the work carefully before releasing
                                                payment.
                                            </p>
                                        </div>
                                    @endif

                                    @include('applications.partials.employer-progress-tracker', [
                                ...
                                <div class="flex items-center justify-between pt-4 border-t">
                                    <div class="flex gap-2">
                                        @if ($application->status === 'pending')
                                            <button type="button" onclick="acceptApplication({{ $application->id }})"
    ...
    && $application->escrow_status === 'not_funded'
)
<form
    action="{{ route('applications.payEscrow', $application->id) }}"
    method="POST"
>
    @csrf
    ...
    <button
        type="submit"
        class="px-4 py-2 bg-[#1e3a8a] text-white rounded-lg hover:bg-[#0f2b5e]"
    >
...
@endif
                                            ...
                                            <button type="button" onclick="rejectApplication({{ $application->id }})"
                                    ...
                                    @if ($application->status === 'pending')
                                        <a href="{{ route('applications.received.view', $application->id) }}"
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <a href="{{ route('jobs.create') }}"
                        class="px-6 py-3 bg-[#1e3a8a] text-white rounded-lg hover:bg-[#0f2b5e] transition-colors">
                        <i class="fa-solid fa-plus mr-2"></i>Post New Job
                    </a>
                </div>
            @endif
