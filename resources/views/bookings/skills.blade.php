                                ...
                                @else
                                    <div class="w-full mt-5 mb-5 pb-5 border-b border-gray-200">
                                        <div class="flex items-center gap-2 flex-wrap">
                                            ...
                                            <button onclick="updateProgress({{ $booking->id }}, 'completed_waiting_payment')"
                                                class="px-3 py-2 rounded-full text-xs font-semibold
                                        ...
                                        @if ($booking->payment_status === 'client_marked_paid')
                                            <div class="mt-4 flex flex-col gap-3">
                                                ...
                                                <div id="dispute-box-{{ $booking->id }}" class="hidden">
                                                    <textarea id="dispute-reason-{{ $booking->id }}" rows="3"
...
<script>
    ...
    function providerPaymentNotReceived(bookingId) {
        ...
        .then(data => {
            if (data.success) {
                alert(data.message || 'Payment dispute submitted. Admin has been notified.');
                location.reload();
            } else {
                alert(data.message || 'Error opening dispute');
            }
        })
