                ...
                <div>
                    ...
                    @else
                        <div class="text-center py-12 bg-gray-50 rounded-xl">
                            ...
                            <p class="text-sm text-gray-400 mt-2">Reviews will appear after completed bookings are rated.
                            </p>
                        </div>
                    @endif
                </div>
                    ...
                    @if ($skill->user)
                        <div class="flex items-center gap-4 mb-4">
                            @if ($skill->user->passport_photo)
                                <img src="{{ asset('storage/' . $skill->user->passport_photo) }}"
                                    alt="{{ $skill->user->first_name }}"
                                    class="w-16 h-16 rounded-full object-cover border-2 border-gray-200">
                            @else
                                <div
                                    class="w-16 h-16 bg-gradient-to-br from-blue-100 to-blue-200 rounded-full flex items-center justify-center">
                                    <span
                                        class="text-lg font-bold text-[#1e3a8a]">{{ substr($skill->user->first_name, 0, 1) }}{{ substr($skill->user->last_name, 0, 1) }}</span>
                                </div>
                            @endif
                            <div>
                                <p class="font-semibold text-gray-900">{{ $skill->user->first_name }}
                                    {{ $skill->user->last_name }}</p>
...
@if (auth()->check())
    @php
        $activeBooking = auth()->id() != $skill->user_id
    ...
    @if (auth()->id() != $skill->user_id)
        ...
        @if (!$activeBooking || $activeBooking->status === 'declined')
            <button onclick="showBookingInterestModal()"
        ...
        @elseif($activeBooking->status === 'confirmed' && $activeBooking->escrow_status === 'not_funded')
            <div class="space-y-3">
                <div class="w-full px-6 py-3 bg-amber-50 text-amber-700 font-semibold rounded-xl border border-amber-200 text-center">
                    Booking confirmed. Please fund escrow so the provider can start work.
                </div>

                <form action="{{ route('bookings.payEscrow', $activeBooking->id) }}"
                      method="POST"
                      onsubmit="showEscrowLoading(this)">
                    @csrf

                    <button type="submit"
                        class="w-full px-6 py-3 bg-[#1e3a8a] text-white font-semibold rounded-xl hover:bg-[#0f2b5e]">
        ...
        @elseif($activeBooking->escrow_status === 'completed')
            <div class="space-y-3">
                <div class="w-full px-6 py-3 bg-blue-50 text-blue-700 font-semibold rounded-xl border border-blue-200 text-center">
                    Provider marked the service as completed. Please confirm or open a dispute from your bookings page.
                </div>

                <a href="{{ route('bookings.requests') }}"
                   class="block w-full text-center px-6 py-3 bg-[#1e3a8a] text-white font-semibold rounded-xl hover:bg-[#0f2b5e]">
                    Go To My Bookings
                </a>
            </div>

        @elseif($activeBooking->escrow_status === 'released' || $activeBooking->status === 'done')
            ...
            @else
                <div class="p-3 bg-emerald-50 border border-emerald-200 rounded-xl mt-3">
                    <p class="text-sm text-emerald-800 text-center">
                        <i class="fa-solid fa-check-circle mr-2"></i>
                        You've already rated this service
                    </p>
                </div>
            @endif

        @elseif($activeBooking->escrow_status === 'disputed')
            <div class="w-full px-6 py-3 bg-red-50 text-red-700 font-semibold rounded-xl border border-red-200 text-center">
                This booking is currently under admin dispute review.
            </div>
        @endif
                        ...
                        <div class="flex gap-2">
                            @if ($booking->status === 'interested')
                                <button onclick="confirmBooking({{ $booking->id }})"
                                    class="flex-1 px-3 py-1 bg-emerald-500 text-white text-xs rounded-lg hover:bg-emerald-600 transition-colors">
                                    Confirm
                                </button>

                                <button onclick="declineBooking({{ $booking->id }})"
                                    class="flex-1 px-3 py-1 bg-red-500 text-white text-xs rounded-lg hover:bg-red-600 transition-colors">
                                    Decline
                                </button>

                            @elseif($booking->status === 'confirmed' && $booking->escrow_status === 'not_funded')
                            ...
                            @elseif($booking->escrow_status === 'funded')
                                <div class="flex-1 px-3 py-1 bg-emerald-50 text-emerald-700 text-xs rounded-lg border border-emerald-200 text-center">
                                    <i class="fa-solid fa-lock mr-1"></i>
                                    Payment secured — manage progress
                                </div>

                            @elseif($booking->escrow_status === 'completed')
                                <div class="flex-1 px-3 py-1 bg-blue-50 text-blue-700 text-xs rounded-lg border border-blue-200 text-center">
                                    Waiting for client confirmation
                                </div>

                            @elseif($booking->escrow_status === 'released')
                                <div class="flex-1 px-3 py-1 bg-green-50 text-green-700 text-xs rounded-lg border border-green-200 text-center">
                                    Payment released
                                </div>
...
@endif
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6">
                        ...
                        <div class="space-y-3">
                            @foreach ($relatedSkills as $relatedSkill)
                                <a href="{{ route('skills.show', $relatedSkill->id) }}"
                                    class="block p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-all">
                                    <div class="flex items-center gap-3">
                                        @if ($relatedSkill->image)
                                            <img src="{{ asset('storage/' . $relatedSkill->image) }}"
                                                alt="{{ $relatedSkill->title }}"
                                        ...
                                        <div class="flex-1">
                                            <p class="font-medium text-gray-900 text-sm">
                                                {{ Str::limit($relatedSkill->title, 30) }}</p>
                                            <p class="text-sm text-[#1e3a8a] font-semibold">
                                                {{ $relatedSkill->formatted_price }}</p>
                                        </div>
                                    </div>
                                </a>
                            @endforeach
            ...
            <div class="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
                ...
                <form id="bookingInterestForm" class="p-6">
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="button" onclick="closeBookingInterestModal()"
                            class="px-4 py-3 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-all">
                            Cancel
                        </button>
                    </div>
    ...
    <script>
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        successDiv.innerHTML =
                            '<i class="fa-solid fa-check-circle"></i><span>Booking request sent successfully!</span>';
        ...
        function showEscrowLoading(form) {
    const button = form.querySelector('button[type="submit"]');

    if (button) {
        button.disabled = true;
        button.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-2"></i>Redirecting to Paystack...';
    }
}
    </script>
